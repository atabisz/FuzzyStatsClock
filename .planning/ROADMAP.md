...
- [x] **Phase 88: GitHub Releases Update Checker** — Once-per-launch GitHub release lookup with accent-colored widget notice and Settings toggle (completed 2026-05-29)
...
- [x] 88-04: Human-verify + close-out — No code. Live dev-box checklist: `#if DEBUG` shows no notice; offline launch is silent; toggle OFF mid-flight cancels and collapses; theme switch + auto-contrast flip both repaint `UpdateText`; SmartScreen/Defender behavior on first outbound HTTPS; absent-field upgrade test from real v4.4 `settings.json`; near-edge re-clamp when notice flips visible; README pass. Pitfall tag: UPD-VERIFY. Requirements: DOCS-01 (final write — README mention).
