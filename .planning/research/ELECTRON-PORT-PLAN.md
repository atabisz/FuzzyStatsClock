...
**Status:** last updated **2026-08-30** — see § Status below. Branch `v5.0-electron-port` at `9667384` plus the uncommitted Phase 5 tree; Phases 3, 4 and 5 closed on Windows, **Phase 5 with one red gate and two claims blocked on an unlocked screen**, Phase 6 next.
...
- **PERF-01 — the v4.4 fade stutter, deferred three times — is closed as a comparative claim, and the architecture is the reason.** The fade interpolates on the **renderer's** `requestAnimationFrame` and writes a composited SVG attribute, so a saturated main process delays *where the fade is going*, never *how smoothly it gets there*. Measured against a v4.4-shaped `setOpacity()`-from-main control built into the same process: the pump holds **median 12.7 ms flat** whether main is idle or blocked 40 ms out of every 50, while the control goes **31.0 → 62.0 ms** `[MEASURED: probe:fade, 8 phases]`. **The absolute figures are provisional** — the desk was locked, so the compositor was not presenting; the comparison is not affected and the numbers get a re-run. § Phase 5.
...
## Status — 2026-08-30 (Phase 5)

Branch `v5.0-electron-port` at **`9667384`** plus the uncommitted Phase 5 tree. Gates, all re-measured for this update: `bun test` **2107 pass / 0 fail** (271,808 `expect()` calls across 43 files), `bun run typecheck` exit 0, `bun run build` exit 0, `bun run probe:display` **51 passed / 0 failed / 10 inconclusive / 0 blocking** across five launches, and **`bun run probe:fade` 7 of 8 blocking arms pass**. ~~At `9532f1b`: 2024 pass / 271,317 / 39 files.~~ ~~At `ff4899d`: 1188 pass / 187,046 / 28 files.~~ ~~At `6370ecc`: 846 pass / 186,489 / 20 files.~~ ~~At `36072c5`: 700 pass / 185,894 / 16 files.~~

**Two gates do not read green, and both are stated here rather than in a phase section, because neither is a defect in this phase's code and both need Alex:**
- **`bun run probe:shell` is 7 passed / 1 failed. S2's `layered` bit is RED** and is deliberately not being relaxed — the correction needs pixel evidence, and the instrument for it (`probe:pixels`, new, the repo's first) cannot run on a locked screen. § Phase 5 carries the measured table that shows S2's *cause attribution* was wrong from Phase 3 onward.
- **`bun run probe:fade`'s F0 is INCONCLUSIVE, so the script exits 1. The workstation was locked for every measurement in this phase.** PERF-01's *comparative* result is unaffected — one process, one host, one load, and the control degraded while the pump did not — but the **absolute** cadence figures are provisional until a run from an unlocked desk. F0 exists because the measured idle median of 12.7 ms is ~79 Hz, which matches no real refresh rate.

**Four probes now answer four different questions, and running a subset hides whichever half it does not reach.** `probe:shell` reads the *window* off Win32; `probe:display` reads the *document* off CDP; `probe:fade` reads a *frame clock* against a negative control; `probe:pixels` reads **an actual pixel off the glass**, which nothing in this tree could do before Phase 5 (`probe-display.ts:64` said so). Phase 4 broke `probe:shell`'s size arm by making the window's size a renderer decision; Phase 5 broke its `layered` arm by removing main's `setOpacity` call. **Both times the expectation was the stale thing — and the second time that could only be established by building a new instrument.**
...
| **5 — Ghost mode** | **DONE on Windows 2026-08-30 — with one RED gate and two lock-blocked claims** | ~~next~~ **PERF-01 is closed as a comparative claim and measured:** the renderer's rAF pump holds median 12.7 ms / p99 13.5 ms whether main is idle or blocked 40 ms in every 50, while a v4.4-shaped `setOpacity`-from-main control in the same process degrades 31.0 → 62.0 ms. **`probe:shell`'s S2 is red** (`layered=false`) and the instrument that can clear it needs an unlocked screen. **The absolute cadence figures are provisional** for the same reason. Three unowned features surfaced; one is paid here, two moved to Phase 6. |
| **6 — Stats panel** | **next** | The Windows parsers exist and are fixture-tested from Phase 1; the mac/linux ones do not. **Grew by two features in Phase 5** — the hover backdrop and the hover fast-refresh, neither of which any phase owned. |
...
**The manual list, which is now four items and is the only place they are collected.** Each is a thing no probe on this host can reach, and three of the four are one physical action each:
1. **Unlock the screen, then run `bun run probe:pixels` and re-run `bun run probe:fade`.** Clears S2 and converts PERF-01's provisional figures into a claim about what a user sees.
...
**Exit status: the exit condition is met and PERF-01 is closed as a *comparative* claim. Two things are not closed, both blocked on a physical action only Alex can take, and neither is a defect in the phase's code.** 8 files added / 6 changed, 3,482 new lines, **83 new tests** (`bun test` **2107 pass / 0 fail**, 271,808 `expect()` calls, 43 files), `typecheck` and `build` exit 0, `probe:display` unchanged at **51 / 0 / 10 / 0**.

| Gate | Result |
|---|---|
| `bun run probe:fade` (new) | **7 of 8 blocking arms pass. F0 is INCONCLUSIVE and therefore the script exits 1** — see § The lock. |
| `bun run probe:shell` | **7 passed / 1 failed. S2 is RED** and stays red — see § S2. |
| `bun run probe:pixels` (new) | **INCONCLUSIVE, exit 0. Nothing was launched and nothing was measured** — see § The lock. |
...
- **The shipped renderer pump: median 12.7 ms, p99 13.5 ms, max 13.8 ms — identical idle and with main blocked 40 ms out of every 50.** Zero frames over the 33.4 ms bar in any phase.
...
#### S2 is a red blocking gate, and it is not being waived
...
**Both need the same thing and it is not a code change:** an unlocked screen, then `bun run probe:pixels` and a re-run of `bun run probe:fade`. Until then S2 stays red rather than being relaxed on unverified grounds.
...
**Exits on:** every one of the **15** cells either shows a live number on its platform or renders `--` through the existing `-1` path, **and both hover behaviours above are wired**. (Was 18 cells — the three temperature cells retired with Option C, matching the table in § The telemetry seam. The two figures disagreeing in one document is exactly how a retired feature comes back as three missing cells.)
