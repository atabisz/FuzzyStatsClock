...
**Status:** last updated **2026-08-30** — see § Status below. Branch `v5.0-electron-port`, Phase 6 committed as the change that carries this update (Phase 5 was `aaa219e`); Phases 3, 4, 5 and 6 closed on Windows, **every gate green including the three that were red or blocked at the last update**, Phase 6.5 / 7 next.
...
## Status — 2026-08-30 (Phase 6)

Branch `v5.0-electron-port`. **Every figure below was measured on the Phase 6 tree while it was still uncommitted, and the commit that carries this document is that same tree** — so they are readings of the change being described, not of the commit before it (`aaa219e`, Phase 5). Gates, all re-measured for this update: `bun test` **2371 pass / 0 fail** (279,263 `expect()` calls across 55 files), `bun run typecheck` exit 0, `bun run build` exit 0. ~~At `9667384`: 2107 pass / 271,808 / 43 files.~~ ~~At `9532f1b`: 2024 pass / 271,317 / 39 files.~~ ~~At `ff4899d`: 1188 pass / 187,046 / 28 files.~~ ~~At `6370ecc`: 846 pass / 186,489 / 20 files.~~ ~~At `36072c5`: 700 pass / 185,894 / 16 files.~~

**Every gate reads green, including the three that did not at the last update.** The screen was unlocked for this run, which is what closed two of them:

| Gate | Result |
|---|---|
| `bun run probe:display` | **61 passed / 0 failed / 10 inconclusive-by-design — 0 blocking**, five launches. Was 51 green; **+10 arms, of which D11b and D6b are new blocking arms** |
| `bun run probe:shell` | **8 passed / 0 failed / 0 inconclusive.** ~~S2 red~~ — S2's `layered` expectation was **corrected on pixel evidence**, not relaxed |
| `bun run probe:pixels` | **3 of 3 blocking arms pass**, X1-X4 all green. ~~INCONCLUSIVE, nothing launched~~ |
| `bun run probe:fade --workers 6` | **8 of 8 blocking arms pass.** ~~F0 INCONCLUSIVE, exit 1~~ — idle median **11.8 ms (~85 Hz) against a 90 Hz panel**, so the absolute figures are now claims about what a user sees |
| `bun run probe:typeperf` | 7 passed / 0 failed / **1 inconclusive (A7, pre-existing)**. First live sample `cpu=21.97 mem=95.87 pag=2.65`, 318 3D engine instances |
| `bun run probe:battery` (new) | **5 passed / 0 failed.** First sample at 1307 ms, mean gap 2045 ms over the run, no child leaked |
| `bun run probe:cost` | 4 passed / 0 failed. **A4 PASS: CPU 9.47% against WPF's 32.19% — 3.40× cheaper.** RSS **INDETERMINATE** again |
| `bun run probe:displays` | 3 passed / **3 failed / 0 blocking** — the recorded ISC-18/ISC-19 diagnostic finding, unchanged |

**Five probes now answer five different questions, and running a subset hides whichever half it does not reach.** `probe:shell` reads the *window* off Win32; `probe:display` reads the *document* off CDP; `probe:fade` reads a *frame clock* against a negative control; `probe:pixels` reads **an actual pixel off the glass**, which nothing in this tree could do before Phase 5 (`probe-display.ts:64` said so); `probe:battery` reads **a live source's cadence over 30 seconds**, which no fixture can, because a parser cannot fail to arrive. Phase 4 broke `probe:shell`'s size arm by making the window's size a renderer decision; Phase 5 broke its `layered` arm by removing main's `setOpacity` call; **Phase 6 broke nothing in `probe:shell` and instead found that one of `probe:display`'s own greens had been asserting a defect as correct behaviour** (D6, the date font). **Three times the expectation was the stale thing, and each time it took a different instrument to establish that.**
...
| **6 — Stats panel + per-platform sources** | **DONE on Windows 2026-08-30; mac/linux are host-blocked, not unwritten** | ~~next~~ 12 modules / 1,980 LOC + 12 test files / 3,454 LOC + a new probe; **`bun test` 2371 pass / 0 fail**, `probe:display` **61 / 0 / 10 / 0**, `probe:battery` **5 / 0**. All three platforms' sources are written and fixture-tested; only Windows has been run live. **A fourth unowned feature surfaced and is the phase's real finding: the uptime line renders five fields in the C# and the port shipped one of them.** |
| **6.5 — Settings window** | **next, unless Alex reorders** | Not in the original phase list. The feature port table listed a second `BrowserWindow`; no phase's exit criteria did, and wiring the tray in Phase 3 walked straight into it. Every setting it edits is already persisted and tray-togglable, so this is a missing editing surface, not missing state. **Phase 6 added two more things that wait on it**: `stats-rows.ts`'s auto-collapse rule, and the re-clamp that a re-shown row needs. |
...
**The manual list, which is now three items and is the only place they are collected.** Each is a thing no probe on this host can reach, and each is one physical action:
...
No probe has seen the **real app** fade under a **real cursor**. Pushing a target needs `ipcRenderer`, which CDP cannot reach; moving the cursor needs `SendInput`, which moves Alex's own. It joins the drag arm and the monitor unplug as **manual**: turn ghost mode on, move the pointer toward the widget, watch it fade, move away, watch it return. Nothing here covers macOS or Linux — `setIgnoreMouseEvents` is one call on all three, and it has been read back off a live window on one. And `FadeGuards.settingsOpen` is wired as a literal `false`, because there is no settings window yet (Phase 6.5).
...
- **`core/stats-rows.ts` is wired to nothing**, deliberately: the only route to per-row toggles in the C# is the settings window, which is Phase 6.5. Phase 6 owns *honouring* the saved flags, and that half is live.
...
| Per-platform parsers | fixture-driven, run on every platform in CI. **All three written and green as of Phase 6** — Windows `typeperf` CSV (both defects fixture-pinned) + PowerShell battery, macOS `vm_stat`/`sysctl`/`pmset`/`ioreg` against **real M1 captures**, Linux `/proc` + `/sys` + `nvidia-smi`. ~~macOS fixtures captured, parsers not written; Linux neither~~ **The Linux fixtures are SYNTHETIC and that is a real gap, not a nick:** a wrong sysfs path is then wrong in the module and in the fixture at once and nothing fails, so both globs log the path they settled on and the test header says so out loud |
| Acquisition | one smoke test per platform in CI matrix (win/mac/linux runners), asserting a plausible range, not a value. **Windows acquisition is MEASURED live; macOS and Linux have never been run at all** — host-blocked, not unwritten, and the distinction is the whole content of Phase 6's exit status |
...
**Buys:** one codebase on three platforms · **a third to a half of the WPF app's CPU under the real workload — 1.93-2.43× cheaper across two Phase 1 runs, and 3.40× on a Phase 6 re-run with the entire stats pipeline live (9.47% vs 32.19% of one core)** `[MEASURED: ISC-6 — this replaces the "~20% of idle CPU, floor only" figure, which was a parked overlay]`. **The Electron number is the durable half of that claim and the ratio is not:** Electron moved 8.21% → 9.47% while gaining two `typeperf` children, a battery poll and a live 1 s panel repaint, whereas the WPF baseline moved **+7.99pp** on the same host between sessions. So the ratio widened because its denominator did — quote the pair from one run, never across runs · a display native to SVG · a plausible fix for PERF-01 · a far cheaper settings UI · auto-contrast's feedback-loop guard reduced from a Z-order walk to one API call (on 2 of 3 platforms).

**Costs:** **2,467 LOC of Core translated — DONE** · **632 tests re-earned, 578 of them after Option C; the 457 Core translations are DONE, and of the App half's 163, the 46 pure-seam cases are DONE** ~~and the App half is not started~~ — of the 117 App cases left, **42 retire with Option C** (`TemperatureServiceTests` 21, `TempsLineTests` 10, 11 temps keys in `AppSettingsTests`; the 12 in `TemperatureFormatterTests` are Core and already counted there), leaving **75** that the fixture parsers and shell probes *replace* rather than translate · 7,605 LOC of `FuzzyClock.App` deleted and ~~~2,500–3,000~~ **6,734** LOC of TS written in its place — **the estimate was out by 2.2–2.7× and it is now measured, not projected**: `src/main` 4,126 + `src/renderer` 2,293 + `src/*.ts` 315, with Phases 6.5 and 7 still ahead of it `[MEASURED: per-file wc -l, 2026-08-30]`. `src/core` is a further **6,408** against the C# Core's 2,467, which is doc comments rather than logic, and **`scripts/` is 5,837 LOC of probes with no C# counterpart at all** — the instruments are a real cost of this port and they were nowhere in this estimate · **three platform telemetry backends where WPF had one, of which only one has ever run on real hardware** · **temperatures retired on all three platforms, a shipped v4.2 feature deliberately given up (Option C)** · GPU unavailable on macOS · auto-contrast unavailable on Linux · Wayland users on XWayland · an Apple Developer ID · three packaging targets with no precedent in his tree · CrowdStrike autostart re-proof · **1.40× more disk than the WPF installer** `[MEASURED: ISC-8]`.
...
| **`bun test` 2371 pass / 0 fail / 279,263 `expect()` calls across 55 files** | `bun test` + `bun run typecheck` + `bun run build`, all on the Phase 6 tree — the same tree this document's own commit carries, re-measured 2026-08-30 |
...
| **F6's band needs `--workers` chosen for the host: 8 workers → 71.5%, 6 → 59.8% against a 20-65% window** | `probe:fade` on this 32-core host. Not a regression and not a waiver — F5 carries the PERF-01 claim regardless |
