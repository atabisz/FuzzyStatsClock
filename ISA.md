...
updated: 2026-08-28T23:37:28+10:00
...
- [~] **ISC-12. ≥457 translated Core tests pass under `bun test`.** Was ≥469; `TemperatureFormatterTests.cs`
  ...
  - **Progress: 335 of the 457 translated, and `bun test` is at 564 pass / 0 fail** (96 from ISC-13 and
    the typeperf fixtures, 468 new). The 335 are `UptimeFormatterTests` 7, `DialGeometryTests` 6,
    ...
    `MultilingualPhraseProviderTests` 128 — each file's
    full case count from the TRX, none partially taken. The other 133 new cases are **additions, not
  ...
  - **122 remain, and all of them are on the phrase side** — **six `.cs` files holding fourteen
    `[TestClass]`es**, per file: `PhraseStyleProviderTests` 9 classes / 64, then five
    single-class expanded files — `English` 13, `Yoda` 12, `Jive` 11, `Pirate` 11, `Terse` 11.
    9 + 5 = 14 classes and 64 + 58 = 122 cases; 335 + 12 retired + 122 = 469. **The file
    ...
    files into 11 translated (22 classes — `PhraseEngineTests.cs` holds two, `SegmentKeyTests.cs`
    four and `MultilingualPhraseProviderTests.cs` eight), 1 retired, and **6 files / 14 classes**
    remaining, so 22 + 1 + 14 = 37 classes and 11 + 1 + 6 = 18 files. Given per file *and* per class on
    ...
    with itself no matter how stale it is.
