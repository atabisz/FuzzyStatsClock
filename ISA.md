...
updated: 2026-08-29T07:13:02+10:00
...
- [~] **ISC-12. ≥457 translated Core tests pass under `bun test`.** Was ≥469; `TemperatureFormatterTests.cs`
  ...
  - **Progress: 399 of the 457 translated, and `bun test` is at 635 pass / 0 fail** (96 from ISC-13 and
    the typeperf fixtures, 539 new). The 399 are `UptimeFormatterTests` 7, `DialGeometryTests` 6,
    ...
    `MultilingualPhraseProviderTests` 128, `PhraseStyleProviderTests` 64 — each file's
    full case count from the TRX, none partially taken. The other 140 new cases are **additions, not
  ...
  - **58 remain, and all of them are on the phrase side** — **five `.cs` files holding five
    `[TestClass]`es**, one class each, per file: `English` 13, `Yoda` 12, `Jive` 11, `Pirate` 11,
    `Terse` 11. 399 + 12 retired + 58 = 469. **The file
    ...
    `[TestClass]` attributes divide 18 files into 12 translated (31 classes — `PhraseEngineTests.cs`
    holds two, `SegmentKeyTests.cs` four, `MultilingualPhraseProviderTests.cs` eight and
    `PhraseStyleProviderTests.cs` nine), 1 retired, and **5 files / 5 classes**
    remaining, so 31 + 1 + 5 = 37 classes and 12 + 1 + 5 = 18 files. Given per file *and* per class on
