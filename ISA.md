...
- [x] **ISC-32.1. `main.ts`'s IPC payload guards refuse a malformed payload, in a real main process, on the
  real relay bodies.** **CLOSED 2026-08-30 — `probe:boundary`, 9 / 0 on win32.** The relay *names* were
  closed by `test/ipc-channels.test.ts`; this is the other half of what the plan calls the relay bodies.
  - **Nothing exercised these guards, and the reason is structural rather than an oversight.** `main.ts`
    imports `electron`, so `bun test` cannot load it, and `probe:settings-window` replaces the relays it
    drives with a recorder — its 37 arms run past guards that are not there. Four handlers validate their
    payload (`resize`, `adjust-opacity`, `hover`, `settings-edit`) and each says why in a comment: *"a
    malformed payload reaching arithmetic is a main-process exception, which takes the whole app down."*
    **There is no `process.on("uncaughtException")` anywhere in `src/main/`**, so that is literal.
  - **A renderer can put a malformed payload on the wire because the preload forwards its argument
    unchanged** and TypeScript's parameter types do not exist at runtime. So
    `window.fuzzyclock.resize("banana")` evaluated over CDP is a real string on the real channel arriving
    at the real guard — the `probe:restart` trick applied to types instead of position. **The threat model
    is a bug, not an attacker**: a renderer computing `NaN` for a size, a refactor that changes an
    argument's type on one side of the bridge only.
  - **B3 is what gives the file teeth.** "The malformed payload had no effect" is also what a relay that
    drops *everything* looks like — a guard inverted to `typeof payload === "number"` would pass every
    silence arm. `NaN` **is** a number, so it passes the relay and is caught one level down by `onResize`,
    which announces itself with `resize: refusing NaNxNaN`. B2 silent **and** B3 loud can only both hold if
    the relay is deciding on the type. The opacity arms split the same way, one relay condition each: B4
    (`"2"`) is the `typeof` half, B5 (`NaN`) is the `Number.isFinite` half.
  - **Two readings, neither checking the other**: main's own stdout, and the backdrop rect's `fill`
    attribute read from the DOM by Chromium. Every silence arm searches **only the stdout written after its
    own send** — a whole-buffer search would let an earlier arm's log satisfy or spoil a later one, quietly
    in both directions.
  - **Three mutations, one per arm group, each leaving the others green.** (1) `hover`'s guard removed →
    **B1 alone**, and the detail is the harm verbatim: `"yes"` is truthy, so the backdrop painted
    (`fill="#00000059"`) as a real enter. (2) `resize`'s width/height `typeof` removed → **B2 alone**;
    the strings reached `onResize` and it logged `resize: refusing bananaxbanana`. (3)
    `adjust-opacity`'s `Number.isFinite` removed, `typeof` kept → **B5 and B8**.
  - **Mutation 2 revised a belief rather than confirming one.** B6 stayed green under it: the app did not
    fall over, because `onResize` has its own `Number.isFinite`/`< 1` check behind the relay. So for
    `resize` the relay guard is **defence in depth, not the only thing between a malformed payload and a
    crash** — the comment's "takes the whole app down" is true of the class and not of that channel.
  - **Mutation 3 is the one where the guard is load-bearing, and `core/opacity-step.ts:51-53` says so
    itself**: `stepOpacity` deliberately carries no finiteness check, on the grounds that "`validateSettings`
    rejects a non-finite opacity before it can reach here" — true of the load path, not of this one.
    Measured consequence: `opacity` on disk became **`null`** (JSON has no NaN), and main logged
    `opacity = NaN (wheel)`. **B8's red is a cascade off B5's and is reported as one** — but the cascade is
    the finding: `stepOpacity(NaN, -1)` is `NaN`, so once wedged, **no subsequent valid wheel event can
    recover it**, and the corrupt value persists across restarts. That is strictly worse than the dropped
    scroll the comment anticipates. **Not measured: whether `validateSettings` heals the `null` on the next
    launch.** It would not change the verdict — the in-session wedge is real either way — so it is left
    open rather than guessed at.
  - **A predicted-true reading that came back false, and it is correct.** B7's cadence line is absent
    because `core/load-average.ts:103` records that `Win32StatsSource.setIntervalSec(0.5)` **declines** the
    hover cadence and keeps the configured interval, so `applyStatsInterval` returns at its
    `adopted === adoptedIntervalSec` check. The backdrop is graded and the cadence is printed as detail for
    exactly this reason (`core/hover.ts:19-21`: enter paints the backdrop unconditionally, and changes the
    interval only when the panel is visible). **So the cadence half of hover is not covered here** — on
    win32 it is not observable through this route at all.
  - **Residual: `settings-edit`'s guard is out of reach**, because the only route to a settings window in
    the running app is `tray?.popUp()`, the shell-owned menu. `onSettingsEdit`'s rejection path is covered
    in core by `applySettingsEdit`; the relay's `typeof id !== "string"` check in front of it is not.
  - **The profile is seeded and B0 is a blocking interlock on `loaded from own-file`**, for
    `probe:restart`'s reason: `SettingsStore.load()` falls back to Alex's live WPF settings when a profile
    has none, and the arms compare against the seeded opacity. Seed also sets `ghostModeEnabled: false`,
    because with ghost on and no modifier held `hoverEnter` returns `NOTHING` and hovering is not an event.
  - **Bounded**: win32. The probe uses only CDP and stdout, so it should run unchanged elsewhere; the B7
    cadence note is the one platform-specific reading and it is not graded.
...
- **A second physical action joined the list with Phase 6.5, and it is one click: right-click the tray
  ...
  **reimplements `main.ts`'s three `ipcMain.on` relays with a recorder in front**, stated at length in its
  ...
  caught it. ~~**What is left is the click and the relay bodies.**~~ **Narrowed again the same day: the
  relay bodies are half-closed by `probe:boundary` (9/0, ISC-32.1), which drives the real `resize`,
  `adjust-opacity` and `hover` guards in a real main process over CDP — the preload forwards its argument
  unchanged, so a malformed payload from the renderer reaches the real guard. What is left is the click, and
  `settings-edit`'s own guard, which is behind that same click.** The tray popup is owned by the shell on all
...
- **The relay-body half of manual item 6 is closed — `probe:boundary`, 9 / 0, ISC-32.1, 2026-08-30.**
  ...
  carry a comment saying a malformed payload "takes the whole app down", **nothing exercised any of them**,
  and there is no `uncaughtException` handler in `src/main/` to soften that. They cannot be unit-tested
  (`main.ts` imports `electron`) and `probe:settings-window` replaces the relays it drives with a recorder.
  - **The way in is that `preload.ts` forwards its argument unchanged and TypeScript's types are gone at
    runtime**, so `window.fuzzyclock.resize("banana")` over CDP is a real string on the real channel. Three
    of the four are reachable this way; `settings-edit` is behind the tray click and stays open.
  - **The discriminator is B3, not the silences.** A guard inverted to `typeof payload === "number"` would
    pass every "nothing happened" arm. `NaN` is a number, so it passes the relay and is caught by
    `onResize`, which says `resize: refusing NaNxNaN` — B2 silent and B3 loud pin the relay to deciding on
    the type. Two readings throughout, main's stdout and the backdrop's `fill` from the DOM, and each
    silence arm searches only the stdout its own send produced.
  - **Three mutations, one per arm group.** `hover`'s guard out → B1 alone, and `"yes"` painted the backdrop
    as a real enter. `resize`'s `typeof` out → B2 alone. `adjust-opacity`'s `Number.isFinite` out → B5 (B8
    cascading off it).
  - **Two findings, one of them smaller than the claim it replaces.** Under mutation 2 the app did **not**
    fall over, because `onResize` has its own finiteness check behind the relay — so for `resize` the relay
    guard is defence in depth rather than the only thing preventing a crash, and the comment's wording is
    true of the class and not of that channel. Under mutation 3 it is the opposite: `core/opacity-step.ts`
    states in its own comment that `stepOpacity` has no finiteness guard because "`validateSettings` rejects
    a non-finite opacity before it can reach here" — true of the load path, not the wire — and the measured
    result was `opacity` **`null`** on disk with `opacity = NaN (wheel)` in the log. Since
    `stepOpacity(NaN, -1)` is `NaN`, no later valid scroll recovers it: one malformed wheel event wedges the
    setting and the wedge outlives the process. Whether the next launch's `validateSettings` heals the
    `null` is **not measured** and would not change the verdict.
  - **A prediction of mine came back false and the reading was right.** B7's hover cadence line is absent
    because `Win32StatsSource.setIntervalSec(0.5)` declines the hover cadence
    (`core/load-average.ts:103`), so `applyStatsInterval` returns before logging. The backdrop is graded and
    the cadence is detail; a probe that graded it would have read a documented decline as a failed hover.
    The cadence half of hover is therefore not covered on win32 at all.
  - `src/` reverted from a `/tmp` backup and all three `dist/` bundles verified back to their pre-mutation
    sha256, so Rule 17 is discharged by construction and no other green is voided. Gates: `probe:boundary`
    9/0, `typecheck` exit 0, `bun test` **2511 / 0 / 280,470 across 59 files — unchanged**, because a probe
    is not a test file. Progress 38/53 → **39/54**.
