...
updated: 2026-08-28T23:41:12+10:00
...
- [~] **ISC-12. ≥457 translated Core tests pass under `bun test`.** Was ≥469; `TemperatureFormatterTests.cs`
  ...
  - **Progress: 207 of the 457 translated, and `bun test` is at 430 pass / 0 fail** (96 from ISC-13 and
    the typeperf fixtures, 334 new). The 207 are `UptimeFormatterTests` 7, `DialGeometryTests` 6,
    ...
    `GetStructuredPhraseTests` 17, `PhraseEngineCoordinatorTests` 17, `SegmentKeyTests` 37 — each file's
    full case count from the TRX, none partially taken. The other 127 new cases are **additions, not
  ...
  - **250 remain, and all of them are on the phrase side** — **seven `.cs` files holding twenty-two
    `[TestClass]`es**, per file: `MultilingualPhraseProviderTests` 8 classes / 128 (French, German,
    Japanese ×4, Polish, Spanish, 16 each), `PhraseStyleProviderTests` 9 classes / 64, then five
    single-class expanded files — `English` 13, `Yoda` 12, `Jive` 11, `Pirate` 11, `Terse` 11.
    8 + 9 + 5 = 22 classes and 128 + 64 + 58 = 250 cases; 207 + 12 retired + 250 = 469. **The file
    and class counts were
    ...
    files into 10 translated (14 classes — `PhraseEngineTests.cs` holds two and `SegmentKeyTests.cs`
    four), 1 retired, and **7 files / 22 classes** remaining. Given per file *and* per class on
    purpose: the two decompositions of
    the same 250 have to agree, and a remainder computed by subtracting a remembered figure agrees with
